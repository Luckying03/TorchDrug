...
本项目选择 BACE 和 HIV 两个公开分子性质预测数据集作为实验对象，二者都来自 MoleculeNet/DeepChem 常用基准。选择这两个数据集的原因在于：BACE 数据规模较小，常用于考察模型在 scaffold split 下对新分子骨架的泛化能力；HIV 数据规模更大，正负样本分布更不均衡，更接近虚拟筛选中“从大量候选分子中找出少量活性分子”的场景。二者共同覆盖了小样本 scaffold 泛化和大规模类别不均衡两类典型药物发现问题。
...
BACE 采用 scaffold split，是因为分子性质预测中随机划分往往会让训练集和测试集包含相似骨架，从而高估模型对未知分子结构的泛化能力。scaffold split 按分子骨架划分数据，测试集更接近真实药物发现中遇到的新结构分子。HIV 按照本次实验设定使用 random split，主要考虑到数据规模较大，且用户希望使用 random 方式与当前实验记录保持一致。
...
四组 MindSpore 核心实验分别对应 BACE-GIN、BACE-GAT、HIV-GIN 和 HIV-GAT。BACE 默认使用 scaffold split，HIV 显式指定 \texttt{--split random}。运行命令如下：
...
这些命令共同构成本文的主要实验矩阵。由于本项目还保留 TorchDrug 对照结果，后续复现结果部分会同时给出 MindSpore 实现和 TorchDrug 框架下的指标，并进一步分析二者差异。
...
\begin{table}[H]
  ...
  \begin{tabular}{l l c c}
    ...
    \midrule
    MindSpore & BACE & +0.0404 & +0.0275\\
    MindSpore & HIV  & -0.0182 & +0.0029\\
...
从 BACE 数据集结果看，MindSpore 版本的 GAT 在测试集上优于 MindSpore 版本的 GIN，Test AUROC 从 0.7103 提升到 0.7507，Test AUPRC 从 0.7174 提升到 0.7449。表~\ref{tab:model-delta} 中可以看到，MindSpore 框架下 GAT 相对 GIN 的 AUROC 提升为 0.0404，AUPRC 提升为 0.0275，提升幅度明显高于 TorchDrug 对照中的 0.0078 和 0.0036。这说明在本项目的 MindSpore 实现和当前 scaffold split 下，注意力机制对 BACE 测试集具有一定帮助。BACE 数据集规模较小，且 scaffold split 会有意让训练集和测试集在骨架结构上存在差异，因此它更接近“新结构分子泛化”场景。GAT 为邻居原子分配不同权重，可能在部分局部结构和官能团识别上比固定求和聚合更灵活。

不过，与 TorchDrug 对照相比，MindSpore 版本的 BACE 结果仍然偏低。GIN 的 Test AUROC 低 0.0913，Test AUPRC 低 0.1130；GAT 的 Test AUROC 低 0.0588，Test AUPRC 低 0.0891。该差距一方面反映出简化复现与成熟平台实现之间仍存在工程细节差异，另一方面也与 BACE 的小样本特性有关。对于小数据集，验证集最优轮次的选择、随机初始化、BatchNorm 统计量和少数测试样本排序变化都会显著影响 AUROC 与 AUPRC。因此，BACE 上的差距不宜简单理解为模型结构完全失效，而应理解为当前复现版本在小样本 scaffold 泛化场景下仍不够稳定。

\subsection{HIV 数据集分析}

从 HIV 数据集结果看，现象与 BACE 不完全相同。MindSpore 版本的测试 AUROC 低于 TorchDrug 对照，但测试 AUPRC 与 TorchDrug 接近，甚至略高。MindSpore GIN 的 Test AUPRC 为 0.3747，高于 TorchDrug GIN 的 0.3652；MindSpore GAT 的 Test AUPRC 为 0.3776，也略高于 TorchDrug GAT 的 0.3738。HIV 是类别不均衡任务，活性分子占比较低，AUPRC 比 AUROC 更能反映模型对正类候选分子的检索能力。换言之，即使 MindSpore 模型在整体正负样本排序上略弱，它仍能在高置信度候选集合中保留较多正类分子，这对虚拟筛选任务具有实际意义。

\subsection{GIN 与 GAT 对比}

从 GIN 与 GAT 的模型对比看，两个框架下的趋势并不完全一致。TorchDrug 中 GAT 在 BACE 和 HIV 上相对 GIN 均有小幅提升；MindSpore 中 GAT 在 BACE 上提升明显，但在 HIV 上 AUROC 低于 GIN、AUPRC 略高于 GIN。这说明 GAT 的收益并不是无条件稳定的。GAT 的 attention softmax 对数值范围、边聚合方式和初始化更敏感，HIV 数据集规模更大、类别更不均衡，模型可能更容易出现“部分正类排序改善但整体排序下降”的情况。相比之下，GIN 的求和聚合更简单，训练过程通常更稳，因此在大规模二分类任务上可能取得更平衡的 AUROC。

\subsection{跨框架差异来源}

MindSpore 与 TorchDrug 结果不完全一致是可以预期的。首先，本项目是教学型简化复现，虽然已经对齐 ordered scaffold split、edge-list 图表示、默认特征、hidden dim、层数、batch size、训练轮数和 best valid AUROC 结果选择，但底层算子、参数初始化、BatchNorm 行为、优化器实现和数据加载方式仍与 PyTorch/TorchDrug 存在差异。其次，TorchDrug 内部对图批处理、scatter/reduce 操作和图神经网络层封装进行了长期优化，而本项目更多使用 MindSpore 基础张量算子手工组织 batch，训练效率和数值细节都可能不同。再次，本项目没有使用预训练分子表示，也没有进行大规模超参数搜索，因此结果不能理解为完整 TorchDrug 平台性能复现，而应理解为对 TorchDrug 分子性质预测核心流程的 MindSpore 任务级复现。

\subsection{实验充分性讨论}

关于是否需要补充实验，从课程大作业角度看，当前实验已经覆盖了两个数据集、两个图神经网络模型、MindSpore 复现结果和 TorchDrug 对照结果，能够支撑“使用 MindSpore 复现 TorchDrug 分子性质预测任务”的基本要求。若希望进一步增强报告说服力，最值得补充的不是重新换模型，而是增加多随机种子实验，例如 seed 0、1、2 三组后报告均值和标准差；其次可以补充 final epoch 与 best valid AUROC 两种选择策略的对比，用来说明结果记录方式对指标的影响；再次可以补充 HIV 上 batch size 256 与 512 的运行时间和指标对比，用来解释为何本项目在 HIV 上使用更大的 batch size。考虑到当前报告已经有四组核心实验和框架对照，本文在结果分析中明确将结论限定为“简化复现”和“任务级复现”，避免对模型性能作过度宣称。

\subsection{课程目标完成情况}

从课程目标角度看，本项目完成了三个层面的工作。第一，在科学任务层面，项目将药物虚拟筛选抽象为分子图二分类任务，并给出了明确的数学定义。第二，在框架实现层面，项目没有直接调用 TorchDrug 训练代码，而是使用 MindSpore 自行实现 GIN、GAT、loss、优化器调用和训练评估循环。第三，在实验验证层面，项目在 BACE 和 HIV 两个数据集上得到了可复现的 AUROC/AUPRC 结果，并与 TorchDrug 框架结果进行了对照分析。总体而言，实验规模虽然不大，但与课程复现任务的范围是匹配的；进一步补实验主要用于提升稳定性论证，而不是弥补任务本身没有完成。
